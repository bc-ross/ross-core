...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(idx) = find(&data[pos..], LFH_SIG) {
        let abs = pos + idx;
        ...
        // LFH fields:
        let file_name_len = u16::from_le_bytes(data[abs + 26..abs + 28].try_into()?) as usize;
        let extra_len = u16::from_le_bytes(data[abs + 28..abs + 30].try_into()?) as usize;

        // File name
        let name_start = abs + 30;
        let name_end = name_start + file_name_len;
        let file_name = String::from_utf8_lossy(&data[name_start..name_end]);

        // File data
        let data_start = name_end + extra_len;

        let compressed_size = u32::from_le_bytes(data[abs + 18..abs + 22].try_into()?) as usize;
        ...
        // Next search continues after this file data:
        pos = data_start + compressed_size;
    }

    if found == 0 {
        eprintln!("No LFHs found!");
    }
