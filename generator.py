...
class Program:
    def __init__(self, name: str):
        self.df = pd.read_pickle(
            pathlib.Path("current_programs").joinpath(
                pathvalidate.sanitize_filename(name).replace(" ", "_") + ".pkl"
            )
        )
...
class CourseSequence:
    def __init__(self, program_names: list[str] | None = None):
        ...
        self.gened_eles = pd.read_pickle(
            pathlib.Path("current_programs").joinpath("General_Education.pkl")
        )

    def validate(self):
        return (
            all(prog.validate_plan(self.df) for prog in self.programs)
            and self.gened_validate()
        )

    def gened_validate(self):
        if (
            self.df.map(lambda x: x.credit if not pd.isna(x) else 0).sum().sum()
            < MIN_REQD_CREDITS
        ):
            return False
        ...
        gened_df = self.df[
            self.df.map(
                lambda x: isinstance(x, GenEdStub) or isinstance(x, GenEdCourse)
            )
        ]
        degree_df = self.df[
            self.df.map(
                lambda x: isinstance(x, DegreeCourse) or isinstance(x, ElectiveCourse)
            )
        ]
